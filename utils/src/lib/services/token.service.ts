...
export class TokenService {
  ...
  private _loadInitialTokenFromStorage(): AuthToken | null {
    ...
    if (token && !this._validateTokenIntegrity(token.accessToken || '')) {
  ...
  public readonly isAuthenticated = computed(
    () =>
      !!this.currentTokenState() && !this.isTokenEffectivelyExpired() && this._validateTokenIntegrity(this.currentTokenState()?.accessToken || ''),
  );
  ...
  constructor() {
    ...
    if (initialToken && !this._validateTokenIntegrity(initialToken.accessToken || '')) {
      this.clearToken();
   ...
   * @private
   */
  private _validateTokenIntegrity(tokenValue: string): boolean {
    try {
      const token = this.currentTokenState();
      if (!token) return false;

      // Recalcula o fingerprint do token atual
      const tokenFingerprint = `${token.accessToken}|${token.issuedAt}|${this.tokenVersion}`;
