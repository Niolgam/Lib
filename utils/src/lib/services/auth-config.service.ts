...
export class AuthConfigService {
  ...
  public readonly passwordValidators = computed(() => {
    ...
    // Adiciona validação de comprimento
    validators.push(Validators.minLength(config.passwordMinLength));

    // Constrói o padrão regex baseado nas configurações
    let pattern = '';
    ...
    if (config.passwordRequireSpecialChars) {
      pattern += `(?=.*[${config.passwordSpecialCharsPattern}])`;
    }

    // Adiciona a regex completa se houver requisitos específicos
    if (pattern) {
      const allowedChars = `[A-Za-z\\d${config.passwordSpecialCharsPattern.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}]`;
      pattern += `${allowedChars}{${config.passwordMinLength},}`;
  ...
  createPasswordStrengthValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (value.length < config.passwordMinLength) {
        errors['minLength'] = {
          required: config.passwordMinLength,
          actual: value.length,
        };
      }
      ...
      if (config.passwordRequireSpecialChars && new RegExp(`[${config.passwordSpecialCharsPattern}]`).test(value) === false) {
